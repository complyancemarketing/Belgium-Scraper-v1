...
export default function Dashboard() {
  ...
  const startScrapingMutation = useMutation({
    mutationFn: () => apiRequest('POST', '/api/uae/scrape/start', { onlyNew: true }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/uae/session'] });
      queryClient.invalidateQueries({ queryKey: ['/api/uae/pages'] });
      toast({
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: (payload: Partial<AppSettings>) =>
      apiRequest('POST', '/api/uae/settings', payload).then((res) => res.json()),
    onSuccess: (data: AppSettings) => {
      queryClient.setQueryData(['/api/uae/settings'], data);
      toast({
  ...
  const stopScrapingMutation = useMutation({
    mutationFn: () => apiRequest('POST', '/api/uae/scrape/stop', {}),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/uae/session'] });
      toast({
  ...
  const resetDataMutation = useMutation({
    mutationFn: () => apiRequest('POST', '/api/uae/reset', {}).then((res) => res.json()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/uae/pages'] });
      queryClient.invalidateQueries({ queryKey: ['/api/uae/session'] });
      toast({
  ...
  const sendTeamsMutation = useMutation({
    mutationFn: () => apiRequest('POST', '/api/uae/teams/send', {}).then((res) => res.json()),
  ...
  const sendDisabled = isScraping || sendTeamsMutation.isPending;
          ...
          <div className="space-y-6">
            ...
            {isScraping && (
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Note: Teams webhook notifications are configured globally on the home page.
                </p>
            </div>
